...
from src.utils.config import HISTGBT_DEFAULT_PARAMS
from src.utils.logger import get_logger

log = get_logger(__name__)


class HistGradientBoostingTriageModel(BaseTriageModel):
    """HistGradientBoosting dla 5-klasowej klasyfikacji triażu MTS."""

    name = "hist_gbt"

    def __init__(self, params: dict[str, Any] | None = None):
        merged = {**HISTGBT_DEFAULT_PARAMS, **(params or {})}
    ...
    def fit(
        ...
        **kwargs,
        ...
        self.train_logger.info(
            f"Trenowanie HistGBT (n={len(X_train):,}, features={X_train.shape[1]})"
        )
        ...
        best_iter = getattr(self.model, "n_iter_", None)
        if best_iter is not None:
            self.train_logger.info(f"Best iteration (early stopping): {best_iter}")
            self.tracker.log_metrics({"best_iteration": best_iter})

        self.train_logger.info(f"Trening HistGBT zakończony w {duration:.1f}s")
